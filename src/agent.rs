...
/// Appended to the summary prompt on the single gate-rejection retry.
const COMPACTION_SUMMARY_RETRY_HINT: &str = "Your previous attempt was rejected: it contained only planning/meta text with no actual summary content. Output the actual summary now, directly, with no preamble or self-description.";
...
/// Counts how many phrases from `phrases` occur in `text` with spans that
/// do not overlap any already-claimed span (recorded in `covered`). Phrases
/// are tried longest-first so a contained variant ("this is a compact"
/// inside "this is a compaction request", "生成一份完整的会话摘要" inside
/// "我需要生成一份完整的会话摘要") never double-counts: the longer phrase
/// claims its span and the contained one is skipped. Each phrase is counted
/// at most once.
fn count_non_overlapping(text: &str, phrases: &[&str], covered: &mut Vec<(usize, usize)>) -> usize {
    let chars: Vec<char> = text.chars().collect();
    let mut ordered: Vec<&str> = phrases.to_vec();
    ordered.sort_by_key(|phrase| std::cmp::Reverse(phrase.chars().count()));
    let mut hits = 0usize;
    for phrase in ordered {
        ...
        while offset + needle.len() <= chars.len() {
            if chars[offset..offset + needle.len()] == needle[..] {
                let span = (offset, offset + needle.len());
                if !covered
                    .iter()
...
/// Self-referential planning phrases that mark a "summary" which only
/// narrates the plan to summarize (a metadiscourse stub) instead of
/// summarizing. Highly specific on purpose: a normal summary may
/// legitimately mention "compaction" (e.g. "the compaction gate was
/// fixed"), so a single phrase is not enough on its own. Returns the number
/// of distinct, non-overlapping phrase hits (Chinese and English keep
/// independent span bookkeeping); the decision — ≥2 hits, or 1 hit combined
/// with missing content signals — lives in `summary_looks_bogus`.
fn metadiscourse_stub_hits(summary: &str) -> usize {
    const ZH: &[&str] = &[
        "这是一个 compaction 请求",
        "这是一个压缩请求",
        "这是一次 compaction",
    ...
    const EN: &[&str] = &[
        ...
        "let me structure",
        "let me compile",
        "triggered by the system prompt",
        "as a compaction",
    ];
    // Both lists match against the same lowercased string (Chinese phrases
    // are case-free, so lowercasing does not disturb them), which keeps all
    // span positions in one coordinate space; the per-language span lists
    // mirror the "Chinese and English have independent intervals" rule.
    let lower = summary.to_lowercase();
    let mut zh_covered: Vec<(usize, usize)> = Vec::new();
    let mut en_covered: Vec<(usize, usize)> = Vec::new();
    let zh_hits = count_non_overlapping(&lower, ZH, &mut zh_covered);
    let en_hits = count_non_overlapping(&lower, EN, &mut en_covered);
    zh_hits + en_hits
}

/// `\b`-style word match: `word` occurs in `text` with a non-alphanumeric
/// (or string-edge) character on both sides. Prevents "files" from matching
/// "file", "latest" from matching "test", and "addressing" from matching
/// "add".
fn contains_word(text: &str, word: &str) -> bool {
    ...
    chars.windows(needle.len()).enumerate().any(|(i, window)| {
...
/// Count of distinct concept-level content words present in `summary` (each
/// list entry counts at most once). English words are matched with word
/// boundaries so plural/verb variants cannot stack; the list is normalized
/// to one representative per concept (file/files → file, fixed/fix → fix,
/// changes/change → change, merged → merge, decided → decision), so a text
/// mentioning both forms still counts the concept once. Chinese words are
/// plain substrings (no word boundaries in CJK); the overlapping variant
/// "重构了" is subsumed by the shorter "重构".
fn content_word_hits(summary: &str) -> usize {
    const CONTENT_WORDS_ZH: &[&str] = &[
        "决策",
        "决定",
        "目标",
        "任务",
    ...
    let zh_words = CONTENT_WORDS_ZH
        .iter()
        .filter(|word| summary.contains(**word))
        .count();
    let en_words = CONTENT_WORDS_EN
        .iter()
        .filter(|word| contains_word(summary, word))
        .count();
    zh_words + en_words
}

/// Commit-hash signal: a contiguous run of ≥7 hex digits that contains at
/// least one a–f (a pure-digit run like "20240812" is a date, not a hash)
/// and whose immediate neighbors are not letters (so "deadbeef" in prose
/// counts, while the hex prefix of "deadbeefx" does not). The run is
/// scanned in full before judging: checking at the 7th digit would misjudge
/// "12345678x" (the run continues past the 7th digit) and runs that only
/// reach ≥7 digits later.
fn has_hash_signal(summary: &str) -> bool {
    ...
    while i < chars.len() {
        if chars[i].is_ascii_hexdigit() {
            ...
            let contains_letter = run.iter().any(|c| matches!(c, 'a'..='f' | 'A'..='F'));
            let prev_ok = start == 0 || !chars[start - 1].is_ascii_alphabetic();
            let next_ok = i == chars.len() || !chars[i].is_ascii_alphabetic();
            if run.len() >= 7 && contains_letter && prev_ok && next_ok {
                return true;
            }
        } else {
            i += 1;
...
/// Content-signal check, used as auxiliary evidence for the metadiscourse
/// gate: true when `summary` carries no concrete artifact — no file path,
/// no commit hash, and fewer than two distinct content words. Deliberately
/// NOT an independent hard condition: a plain conceptual summary (no
/// paths, no hashes, no vocabulary words) is a perfectly valid summary, so
/// this only strengthens a single metadiscourse phrase into a rejection.
fn lacks_content_signals(summary: &str) -> bool {
    // File-path signal: a path separator or a known source/config
    // extension.
    const EXTENSIONS: &[&str] = &[".rs", ".toml", ".md", ".py", ".go", ".ts", ".sh", ".json"];
    if summary.contains('/')
        || summary.contains('\\')
        || EXTENSIONS.iter().any(|ext| summary.contains(ext))
    {
        return false;
    }
    if has_hash_signal(summary) {
        return false;
    }
    content_word_hits(summary) < 2
}
...
/// the compacted request tail, a too-short stub, or a metadiscourse stub
/// that only narrates the plan to summarize (≥2 self-referential phrases,
/// or a single one with no concrete content signals) — rather than a
/// genuine summary. Conservative on purpose: a false positive only fails
/// the compaction (which is retried once, then on the next auto/manual
/// compact), while a false negative would persist a poisoned summary into
/// every future request. Content signals are deliberately NOT an
/// independent hard condition: a plain conceptual summary with no file
/// paths, no hashes, and no vocabulary words passes as long as it carries
/// no metadiscourse phrases.
fn summary_looks_bogus(
    ...
    // (d) Metadiscourse stub: prose that only narrates the plan to
    // summarize ("this is a compaction request, let me organize the
    // timeline"). Phrases are counted without overlap, so a single phrase
    // ("this is a compaction request") is exactly one hit even when a
    // contained variant ("this is a compact") is present. Two or more hits
    // are pure metadiscourse and always rejected; a single hit is only
    // rejected when the summary also lacks concrete content signals
    // (paths / hashes / content words). Everything else — including plain
    // conceptual summaries with no phrases at all — passes.
    let stub_hits = metadiscourse_stub_hits(summary);
    if stub_hits >= 2 {
        return Some(format!(
            "summary is a metadiscourse stub ({stub_hits} self-referential planning phrases)"
        ));
    }
    if stub_hits == 1 && lacks_content_signals(summary) {
        return Some(
            "summary is a metadiscourse stub (1 self-referential planning phrase) and lacks content signals (no file paths, no commit hashes, fewer than 2 content words)"
                .to_string(),
        );
    }
    None
...
impl Agent {
    ...
    pub(crate) async fn prepare_compaction(&mut self) -> anyhow::Result<CompactionOutput> {
        ...
        let request_tail = request_tail_text(&context[..split]);
        ...
        // Attempt 0 uses the plain prompt; a sanity-gate rejection gets one
        // retry (attempt 1) with the reinforced prompt appended to the same
        // request. Hard failures (tool calls, empty content) bail
        // immediately on any attempt; only the gate rejection retries. The
        // reported usage accumulates every attempt's usage (each
        // `model.complete` returns its own), so a success after a retry
        // does not silently drop the first call's tokens.
        let mut total_usage: Option<Usage> = None;
        let mut last_reason = None;
        for attempt in 0..2 {
            if attempt == 1
                && let Some(Message::User { content, .. }) = request.last_mut()
            {
                content.push(' ');
                content.push_str(COMPACTION_SUMMARY_RETRY_HINT);
            }
            let (response, usage) = model.complete(&request, &[], Some(&mut on_delta)).await?;
            if let Some(usage) = usage {
                let accumulated = total_usage.get_or_insert(Usage::default());
            ...
            if let Some(reason) = summary_looks_bogus(&summary, retained, request_tail) {
        ...
        anyhow::bail!(
            "compaction summary rejected by sanity gate: {}",
