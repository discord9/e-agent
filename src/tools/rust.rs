...
pub fn run_rust_tool(workspace: &Workspace, sandbox: Option<&crate::config::Sandbox>) -> Box<dyn Tool> {
    Box::new(RunRust { workspace: workspace.clone(), policy: run_rust_policy(sandbox), compile_timeout: COMPILE_TIMEOUT, run_timeout: RUN_TIMEOUT, #[cfg(test)] scratch: None })
}
#[rustfmt::skip]
