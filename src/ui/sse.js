...
function restoreUsageFromSnapshot(events) {
  ...
  for (const ev of events) {
    if (!ev) continue;
    if (ev.type === "notice" || ev.type === "display") {
      const text = typeof ev.data === "string" ? ev.data : ev.data && (ev.data.text || ev.data.message);
...
function handleSSEBlock(block, id, wsId, epoch) {
  ...
  if (eventName === "resync") {
    ...
    const NAME = {
      ...
      notice: "Notice", display: "Display", error: "Error",
