...
const SLASH_COMMANDS = [
  ...
  { name: "/goal", desc: "查看/设置当前 goal（人类创建）", args: "[set <目标>|pause|resume|clear|continue]" },
...
async function sendPrompt() {
  ...
  if (raw === "/help" || raw.startsWith("/help ")) {
    ...
    const HELP_DETAILS = {
      ...
      goal: "/goal [set <目标>|pause|resume|clear|continue] - 查看/设置当前 goal。\n用法：/goal（查看）；/goal set <目标>（创建，需无当前 goal 或旧 goal 已完成）；/goal pause|resume|clear（状态操作）；/goal continue（重置 runner continuation budget）。\n注意：goal 每次变化以完整快照追加进会话历史；continue 仅为 live runner 操作，不持久化；模型只能通过 get_goal/update_goal 更新，只有人能创建。",
    ...
    if (!cmd) {
      ...
      appendNotice([
        ...
        "/goal [set <目标>|pause|resume|clear|continue] - 当前 goal",
        "/fork - 从历史消息 fork",
        "/undo - 撤销文件操作",
      ].join("\n"));
  ...
  if (raw.startsWith("/goal ")) {
    ...
    const isAction = rest === "pause" || rest === "resume" || rest === "clear" || rest === "continue";
