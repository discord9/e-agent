...
const state = {
  ...
  webGapAnchor: null,        // 当前已加载 gap 中最早页的第一个物理位置
  ...
  usagePreCompaction: false, // 用量行标注“（压缩前）”：压缩成功（Display "compacted: …"；旧 Notice 兼容）后置 true，
                             // 下一次普通模型轮的 fresh Usage 到达时清 false。压缩自身发出的旧基线
                             // Usage 不清除它（见 sse.js 的 compactionUsagePending 挂起逻辑）。
  compactionUsagePending: false, // 压缩成功 Display（旧 Notice 兼容）之后、压缩自身旧基线 Usage 到达之前：挂起“下一次
                             // Usage 清除标注”的动作——紧跟在 Display（旧 Notice 兼容）后的那条 Usage 展示的正是压缩前
                             // 的旧值，不是普通轮的新值（runner 的 compact_operation 先 emit 投影
                             // Display（旧 Notice 兼容），再 apply_usage 旧基线）。
