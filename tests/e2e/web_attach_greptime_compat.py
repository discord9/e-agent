#!/usr/bin/env python3
"""Isolated GreptimeDB product compatibility probe: new -> bca5941 -> new.
...
import web_attach_greptime_product as h

ROOT = Path(__file__).resolve().parents[2]
ART = ROOT / ".e-agent" / "greptime-web-acceptance"
NEW = h.BINARY
CONN = os.environ.get("GREPTIME_E2E_CONN", "")
...
def main():
    if "127.0.0.1" not in CONN:
        raise SystemExit("GREPTIME_E2E_CONN must name isolated localhost GreptimeDB")
    ...
    before = {"new": {"path": str(NEW.resolve()), "sha256_before": sh(NEW)},
              ...
              "old_expected_sha256": OLD_EXPECTED, "server_isolation_overrides": h.safe_env(env), "greptime_conn": CONN, "stages": []}
