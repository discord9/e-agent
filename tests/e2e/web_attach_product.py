#!/usr/bin/env python3
"""Product acceptance for web attach using the shipped --serve binary.
...
ARTIFACT_ROOT = ROOT / ".e-agent" / "web-product-acceptance"
BINARY = Path(os.environ.get("EAGENT_PRODUCT_BINARY", "/mnt/nvme_rust/rust-targets-2/e-agent-web-attach/debug/e-agent"))
...
def main():
    ...
    provenance = {"case": args.case, "binary": {"resolved_path": str(BINARY.resolve()), "sha256_before": sha256(BINARY)},
                  "server_argv": argv, "server_isolation_overrides": safe_env(env),
