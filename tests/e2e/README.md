...
│   ├── sidebar.py       # 侧边栏：开合、会话树（活跃/历史分组 + label 优先）、筛选、8 条限制+展开、
│   │                    #         挤压布局（桌面右移 280px）、持续打开 + localStorage 持久化
│   ├── chat.py          # 聊天：openSession、状态保留（草稿/滚动）、SSE 基本流、错误渲染、
│   │                    #         并发写冲突现状（Failed->失败）
│   ├── commands.py      # 命令：/compact、/rename、/btw 斜杠拦截（mock 端点）
│   ├── listview.py      # 列表：标题两行、搜索、旧 server 降级、inactive 恢复、删除、📌 置顶
│   ├── mobile.py        # 手机视口 390px 不横向溢出（覆盖式侧边栏）
...
| `sidebar_squeeze` | 桌面挤压布局：打开时内容右移 280px、遮罩 display:none、点内容区不关 |
| `sidebar_persist` | 持续打开：切会话/返回列表保持 + localStorage 跨刷新恢复 |
...
| `conflict_card` | 并发写冲突现状：Failed→「失败」chip、冲突错误消息「错误: 」行（无 .conflict 卡片） |
...
| `list_pin` | 📌 置顶（列表 + 侧边栏树）：PUT /api/sessions/{id}/pin body、.pinned/📌 高亮、旧 server 404 提示不崩 |
...
- **TODO 登记**（`cases/skipped.py`）：当前为空。曾登记的 4 个 TODO 已全部合入 main
  并转正（sidebar_squeeze / sidebar_persist / list_pin / conflict_card——conflict_card 按
